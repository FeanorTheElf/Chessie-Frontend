import {ChessGame, ChessMove, Color, getColor, Chesspiece, PieceType, getPieceType} from "../logic/Chess";
...
function getBestMove(state: ChessGame, depth: number): ChessMove{
...
self.onmessage = function(ev: MessageEvent){
    let state: ChessGame = ChessGame.deserialize(ev.data.state);
    let depth: number = ev.data.depth;
    (postMessage as Function).call(self, getBestMove(state, depth).serialize());
};